...
findMinimalGraphSet = function(n = 9L, r = 3L, s = 4L, 
                               maxOrder = 6L, symRS = FALSE, lowerOnly = FALSE, minAuto = NULL, factor = FALSE,
  ...
  while (any(is.na(dispensable))) {
    ...
    altRes  = iterateLPProof(n = n, r = r, s = s, 
                             maxOrder = maxOrder, symRS = symRS, lowerOnly = lowerOnly, minAuto = minAuto, factor = factor, 
                             ...
                             custom = custom, eps = eps, strongCuts = strongCuts, star = star, inds = altInds)
    if (!is.null(altRes[[4]])) {
...
### Essential parameters are n, r, and s; all of them are assumed to be integers.
...
### If custom is TRUE, only uses the Custom.RData file to extract the graphs instead.

### Additional parameters (apply to the process):
...
### If star is k (not NULL), the process terminates once a bound is found for K_{1,k}.
...
iterateLPProof = function(n = 6L, r = 3L, s = r, 
    ...
    if (improved) {
      ...
      for (ind in 1:length(fullResult)) {
        ...
        if (!is.null(curResult$proofL)) {
          ...
          if (!is.null(star) && curSize == star && all(allGraphs[[curGraph]] == cbind(1:star, star + 1))) {
            contradict = TRUE
            specialIndex = curGraph
            break
          }
        }
  ...
  contradictions = NULL
  if (contradict) {
    contradictions = reconstructContradictions(proofs, boundTab, n, specialIndex = specialIndex)
  }
...
eliminateRedundantBounds = function(allGraphs, boundTab, curGraphTab, support, allGraphAdj, allGraphOrb) {
  ...
  keepAll = FALSE
  while (n_distinct(boundTab %>% filter(!subsumed) %>% pull(graph)) > 1 && !keepAll) {
